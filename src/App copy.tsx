...
function App() {
  ...
  supportedBibles.length === 0 ? (
    <div className="text-sm text-gray-500">Loading versions…</div>
  ) : (
    supportedBibles.map((v) => (
      <button
        key={v.id}
        onClick={() => { setBibleVersion(v.abbreviation); setSelectedBibleId(v.id); }}
        className={`w-full p-3 rounded-lg text-left transition-colors ${
          bibleVersion.toUpperCase() === v.abbreviation.toUpperCase()
            ? 'bg-green-600 text-white'
            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
        }`}
      >
        <div className="font-medium">{v.abbreviation}</div>
        <div className={`text-sm ${bibleVersion.toUpperCase() === v.abbreviation.toUpperCase() ? 'text-green-100' : 'text-gray-500'}`}>
          {v.name}
        </div>
      </button>
    ))
  ...
  (bibleVersions[bibleLanguage as keyof typeof bibleVersions] || []).map((version) => (
    <button
      ...
      className={`w-full p-3 rounded-lg text-left transition-colors ${
        bibleVersion === version.code ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
      }`}
    >
      <div className="font-medium">{version.name}</div>
      <div className={`text-sm ${bibleVersion === version.code ? 'text-green-100' : 'text-gray-500'}`}>
        {version.description}
      </div>
    </button>
  ))
